...
export default function AlertMessage({ alert, onClick }) {
  let message;
  if (Array.isArray(alert)) {
    message =
      alert.length === 1
        ? 'There is one form error.'
        : `There are ${alert.length} form errors.`;
  } else {
    message = alert;
  }
  ...
  return (
    <>
      {message && (
        <div className="mb-8 flex max-w-3xl items-center justify-between rounded-sm bg-red-400">
          <div className="flex items-center">
            ...
            <div className="py-4 text-sm font-medium text-white">{message}</div>
          </div>
          <button type="button" className="group mr-2 p-2" onClick={onClick}>
            <svg
              className="block h-2 w-2 fill-red-800 group-hover:fill-red-800"
              xmlns="http://www.w3.org/2000/svg"
              width="235.908"
              height="235.908"
